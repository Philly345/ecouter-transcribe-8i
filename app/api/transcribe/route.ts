...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "Upload response text (first 200 chars):",
      ...
      if (isHtmlResponse(responseText)) {
        ...
        if (
          ...
          return createErrorResponse(
            "File too large for upload. Please use a smaller file (max 25MB).",
            413
          );
        }
        if (
        ...
        return createErrorResponse(
          "Upload service returned an error. Please try again or contact support.",
          502
        );
      }
      ...
      if (!uploadResponse.ok) {
        ...
        if (uploadResponse.status === 413) {
          return createErrorResponse(
            "File too large for upload. Please use a smaller file.",
            413
          );
        }
        return createErrorResponse(
    ...
    try {
      ...
      const summaryPrompt = `Please analyze this transcript and provide a structured response:\n\nTRANSCRIPT: "${transcript.text.substring(0, 8000)}"${
        transcript.text.length > 8000 ? "...[truncated]" : ""
      ...
      if (geminiResponse.ok) {
        ...
        if (generatedText) {
          ...
          const topicsMatch = generatedText.match(
            /TOPICS:\s*(.+?)(?=INSIGHTS:|$)/s
          );
